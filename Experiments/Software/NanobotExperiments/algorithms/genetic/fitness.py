"""
.. module:: genetic.fitness
    :platforms: Unix, Windows
    :synopsis: Fitness calculations for the Genetic Algorithm
...
# Initialise a logger
logger = logging.getLogger('GeneticFitness')
logging.basicConfig(
    level=logging.INFO,
...
def _check_max_absorbance_above_cutoff(
    max_abs: float, threshold: float
) -> bool:
...
def _check_no_peaks_after_wavelength_cutoff(
    x: np.array, y: np.array, threshold: float
) -> bool:
...
def maximise_single_peak(x, y, target_nm, sigma=20):
    ...
    Returns:
        fitness (float): Fitness of the given data as a measure of distance
                            from target
    """
    x = np.array(x)
    y = np.array(y)
...
def calculate_region_area(
    ...
    start = np.searchsorted(x, start_nm, "left")
    end = np.searchsorted(x, end_nm, "right")
...
def rods_fitness(x: list, y: list) -> float:
    ...
    Passes thourgh a series of filters to correct fitness to account
    for bad results
    ...
    logger.debug(f'Area of Transverse: {transverse}')
    logger.debug(f'Area of Middle region: {middle}')
    logger.debug(f'Area of Long: {long}')
    logger.debug(f'Total peaks: {len(peaks)}')
    logger.debug(f'Peak locations: {x[peaks]}')

    penalty_count = 0

    # Low absorbance, return 0
    if np.max(y) < 0.50:
        logger.info('Penalty -- Below absorbance threshold -- 0% fitness')
        return 0

    # More than 4 peaks
    if len(peaks) >= 4:
        logger.info('Penalty -- More than 4 peaks -- 0% fitness')
        return 0

    # Less than two peaks, 10% of calculated fitness
    if len(peaks) < 2:
        logger.info('Penalty -- Less than two peaks -- 40% fitness')
        fitness *= 0.4
        penalty_count += 1

    # Area of the rods is less than the transverse peak, fitness = 0
    if long < transverse:
        logger.info('Penalty -- Long less than transverse -- 50% fitness')
        fitness *= 0.5
        penalty_count += 1

    # Middle peak is greater than the transverse peak, 75% calculated fitness
    if middle > transverse:
        logger.info('Penalty -- Middle greater than transverse -- 75% fitness')
        fitness *= 0.75
        penalty_count += 1
...
def octahedron_fitness(x: list, y: list) -> float:
    ...
    if unwanted_region > target_region:
        logger.info('Penalty -- Unwanted greater than target -- 0% fitness')
        return 0

    # More than 4 peaks
    if len(peaks) >= 2:
        logger.info('Penalty -- More than 3 peaks -- 60% fitness')
        fitness *= 0.6
